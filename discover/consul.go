...
//实现原本定义的client接口，适配对应的consul交互

type InstanceInfo struct {
	ID string `json:"ID"` //实例id
	Name string `json:"Name"` //服务名称
	Service string `json:"Service,omitempty"` //服务发现时获取的服务名//此处不能有空格
	Tags []string `json:"Tags,omitempty"`//标签，用于服务过滤【？】
	Address string `json:"Address"` //服务host
	Port int `json:"Port"`//相应端口
...
func (consulClient *MyDiscoverClient) Register(serviceName, instanceId, healthCheckUrl string,instanceHost string, instancePort int, meta map[string]string, logger *log.Logger) bool {
	...
	//fmt.Println("http://"+consulClient.Host+":"+strconv.Itoa(consulClient.Port)+"/v1/agent/service/register")
	if err == nil {
